...
function isFinalEntry () {
  log();
  if (eof && !!!queue.node.length() && !!!queue.way.length()  && !!!queue.relation.length()) {
    timing();
...
function pauseHandler (file) {
  var d = '----------'
  var ispaused = false;
  memwatch.on('stats', function(stats) {
    var cap = options.memcap;
    memUse = stats.current_base / 1024;

    if (file && memUse > cap && !ispaused) {
      console.log(d+'pause'+d)
      ispaused = true;
      file.pause();
      setTimeout(function () {
        console.log(d+'resume'+d);
        ispaused = false;
        file.resume();
      }, 2000);
    }
...
function parse (xmlNode)  {
  switch(xmlNode.name)
  {
    ...
    case "nd":
      .push(parseInt(xmlNode.attributes.ref.value, 10));
      break;
...
function closetag (tagName) {
  switch(tagName)
  {
    ...
    case "way":
      ...
      if (!!!queue.way.concurrency && !!!queue.node.length()) {
    ...
    case "relation":
      ...
      if (!!!queue.relation.concurrency && !!!queue.way.length()) {
