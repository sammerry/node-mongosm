...
function log() {
  var savedNode =  queue.output.node || 0;
  var queuedNode = queue.input.node || 0;
  var savedWay =  queue.output.way || 0;
  var queuedWay = queue.input.way || 0;
  ...
  lastReadNodes = savedNode
  lastReadWays = savedWay
}
...
function finalEntry () {
  log();
  process.stdout.write("Final Node Of Type: ")
  timing();
}
...
mongoose.connection.on('open', function () {
  ...
  var intervalId = setInterval(log, 3000);
